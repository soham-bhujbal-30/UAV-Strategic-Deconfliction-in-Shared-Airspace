...
def _convert_mission_to_trajectory(mission: PrimaryMission) -> List[TimedWaypoint]:
    """
    Converts a PrimaryMission (3D waypoints + speed) into a discrete
    time-based 3D trajectory (list of TimedWaypoints).
    """
    trajectory = []
    ...
    for i in range(len(mission.waypoints) - 1):
        ...
        # calculate_distance is now 3D, so this is the true 3D distance
...
def _find_drone_position(trajectory: List[TimedWaypoint], current_time: float) -> Optional[Waypoint]:
    """
    Finds a drone's 3D (x, y, z) position at a specific time from its trajectory.
    (This function doesn't need changes, as it relies on interpolate_position, which we updated)
    """
...
def check_for_conflicts(
    ...
    (This function doesn't need changes, as all the functions it calls
    are now 3D-aware!)
    """
    ...
    while current_time <= mission_end:
        ...
        for sim_flight in simulated_flights:
            ...
            # This is now a 3D distance check
